...
public class GoalsController(IGoalService goalService, IGoalAnalyticsService goalAnalyticsService, ICurrentUserService currentUser) : ControllerBase
{
    ...
    /// <summary>Retrieves all goals</summary>
    ...
    {
        var userId = _currentUser.GetUserId();
        await _goalService.UpdateGoalAsync(userId, id, dto);
        return NoContent();
    }
    ...
    {
        var userId = _currentUser.GetUserId();
        var result = await _goalAnalyticsService.GetGoalHeatmapAsync(userId, goalId, query.Days);
        return Ok(result);
    }
}
